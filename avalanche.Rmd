...
- Historically, this location was well within the Actual
  Evapotranspiration (AET)/Climatic Water Deficit (CWD) space that WBP
  inhabits in the GYE and likely was a favorable environment for the
  species.  Climate projections show increases in CWD and AET across
  the site on average, with a future climate somewhere between
  high-AET/moderate-CWD to extreme-AET/high-CWD relative to the
  historical climate at other locations currently inhabited by WBP in
  the GYE (Figure \@ref(fig:core-areas)).  However, when examined at a
  fine scale using 1m LiDAR DEM data, variability in topoedaphic
  features creates a wide range of microclimates that can be selected
  to buffer against projected climate extremes.
- The 'worst-case' CWD scenario shows annual CWD far exceeding the
  1988 drought year CWD, on average.  2/4 scenarios show similar
  annual CWD on average to the 1988 annual CWD.  The 'best-case' CWD
  scenario shows slight increases in average annual CWD across the
  site (Figure \@ref(fig:climate-summary-2)).
- All projections show an increase in annual AET across the site
  (Figure \@ref(fig:climate-summary-1)).  Annual AET is not likely to
  be a limiting factor for plantings here, and high annual AET can
  potentially be detrimental depending on the timing of the available
  energy and water, for example if the high AET levels are experienced
  mainly during a short window of the year.
- Locations buffering from high to extreme CWD could promote seedling
  survival.  These positions are evident in the darker purple areas of
  figures \@ref(fig:historical-2), \@ref(fig:drought-year-2), and
  \@ref(fig:avalanche-projections-2), and are mainly defined by N-NE
  aspect and/or presence of the soil type with higher soil WHC.
- Based on available soil survey data, soils are relatively
  homogeneous across the polygon (Figure \@ref(fig:soils)). However, a
  pocket of deeper soil located on the western edge of the polygon
  offers soils with higher water holding capacity, which
  simultaneously increases AET and decreases CWD compared to similar
  positions with lower soil WHC.
- If planting outside of the polygon is possible, there is evidence of
  several positions on the landscape that offer relative protection
  from extreme dryness (Figures \@ref(fig:drought-year-2),
  \@ref(fig:avalanche-projections-2)) .
...
avalanche_dir <- file.path("/media/smithers/shuysman/data/out/nps-wb/avalanche/")
avalanche_historical_dir <- file.path(avalanche_dir, "historical/sums/")
avalanche_projections_dir <- file.path(avalanche_dir, "projections/sums/")

avalanche_historical_aet <- rast(list.files(avalanche_historical_dir, pattern = ".*AET.*.nc", full.names = TRUE)) %>%
        clamp(lower = 0, values = NA) / 10
avalanche_historical_cwd <- rast(list.files(avalanche_historical_dir, pattern = ".*Deficit.*.nc", full.names = TRUE)) %>%
        clamp(lower = 0, values = NA) / 10
...
ggplot() +
    ...
    geom_sf(data = avalanche_poly, fill = NA, lwd = 1.5) +
    labs(title = "2002-2022 Average Annual AET", x = "", y = "") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
...
ggplot() +
    ...
    geom_sf(data = avalanche_poly, fill = NA, lwd = 1.5) +
    labs(title = "2002-2022 Average Annual CWD", x = "", y = "") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
...
ggplot() +
    ...
    scale_fill_viridis_c(option = "D") +
    geom_sf(data = avalanche_poly, fill = NA, lwd = 1.5) +
    labs(title = "1988 Annual AET (Drought Year)", x = "", y = "") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
...
ggplot() +
    ...
    scale_fill_viridis_c(option = "B") +
    geom_sf(data = avalanche_poly, fill = NA, lwd = 1.5) +
    labs(title = "1988 Annual CWD (Drought Year)", x = "", y = "") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
```
...
avalanche_aet_warm_wet <- load_gcm_mean(file.path(avalanche_projections_dir, "MRI-CGCM3_rcp85_AET_annual_sum.nc"))
avalanche_aet_hot_wet <- load_gcm_mean(file.path(avalanche_projections_dir,"CanESM2_rcp85_AET_annual_sum.nc"))
avalanche_aet_warm_dry <- load_gcm_mean(file.path(avalanche_projections_dir, "MRI-CGCM3_rcp45_AET_annual_sum.nc"))
avalanche_aet_hot_dry <- load_gcm_mean(file.path(avalanche_projections_dir,"HadGEM2-CC365_rcp85_AET_annual_sum.nc"))
...
avalanche_cwd_warm_dry <- load_gcm_mean(file.path(avalanche_projections_dir, "MRI-CGCM3_rcp45_Deficit_annual_sum.nc"))
avalanche_cwd_hot_dry <- load_gcm_mean(file.path(avalanche_projections_dir,"HadGEM2-CC365_rcp85_Deficit_annual_sum.nc"))
avalanche_cwd_warm_wet <- load_gcm_mean(file.path(avalanche_projections_dir, "MRI-CGCM3_rcp85_Deficit_annual_sum.nc"))
avalanche_cwd_hot_wet <- load_gcm_mean(file.path(avalanche_projections_dir,"CanESM2_rcp85_Deficit_annual_sum.nc"))
...
writeRaster(avalanche_aet_2022, "for_libby/avalanche_aet_2002-2022.tif", overwrite = TRUE)
writeRaster(avalanche_cwd_2022, "for_libby/avalanche_cwd_2002-2022.tif", overwrite = TRUE)
writeRaster(avalanche_aet_hot_wet_change, "for_libby/avalanche_aet_hot_wet_change.tif", overwrite = TRUE)
writeRaster(avalanche_cwd_hot_dry_change, "for_libby/avalanche_cwd_hot_dry_change.tif", overwrite = TRUE)
writeRaster(avalanche_aet_hot_wet, "for_libby/avalanche_aet_hot_wet.tif", overwrite = TRUE)
writeRaster(avalanche_cwd_hot_dry, "for_libby/avalanche_cwd_hot_dry.tif", overwrite = TRUE)
